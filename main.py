...
Duplication_FILE = "last_duplication.json"
def main():

    last_data_fetch = datetime.min.replace(tzinfo=timezone.utc)

    print("=== 🕐 Statement Fetcher with Daily & Weekly Reports ===")
    os.environ["username"] = input("Enter your username (client_id): ").strip()
    os.environ["password"]  = input("Enter your password (client_secret): ").strip()
        ...
        # Get new data
            ...
            if records:
                ...
                for curr, nxt in zip(records, records[1:]):
                    if curr["transactionId"] > 0 and nxt["transactionId"] > 0:
                        ...
                        for record in records:
                            ...
                            if time_str:
                                try:
                                    ...
                                    if day_start <= tx_time  and tx_time < day_end:
